...
final class Loader {
   ...
   */
  public static $sections = array('system');
  /**
  ...
  public static function search($file) {
    foreach(array_reverse(self::$sections) as $section) {
      ...
      $filename = ENVIRONMENT_ROOT.DIRECTORY_SEPARATOR.$section.DIRECTORY_SEPARATOR.$file.FILE_EXTENSION;
      if(file_exists($filename)) {
        return $filename;
      }
      // Does file exist as lowercase?
      $filename = ENVIRONMENT_ROOT.DIRECTORY_SEPARATOR.$section.DIRECTORY_SEPARATOR.strtolower($file).FILE_EXTENSION;
      if(file_exists($filename)) {
        return $filename;
      }
    }
  ...
  public static function multi_search($file) {
    $arr = array();
    foreach(array_reverse(self::$sections) as $section) {
      // Does file exist in exact case supplied?
      $filename = ENVIRONMENT_ROOT.DIRECTORY_SEPARATOR.$section.DIRECTORY_SEPARATOR.$file.FILE_EXTENSION;
